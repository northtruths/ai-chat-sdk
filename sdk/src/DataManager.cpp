    ...
    , db_(nullptr)
{
    ...
    if (rc != SQLITE_OK) {
        LOG_ERROR_STREAM() << "打开数据库失败: " << sqlite3_errmsg(db_);
        return;
    }
    LOG_INFO_STREAM() << "数据库打开成功: " << db_path_;

    if (!init_database()) {
        LOG_ERROR_STREAM() << "初始化数据库失败";
        sqlite3_close(db_);
        db_ = nullptr;
    }
}
...
bool DataManager::execute_sql(const std::string& sql) {
    ...
    if (rc != SQLITE_OK) {
        LOG_ERROR_STREAM() << "SQL执行失败: " << errmsg;
        sqlite3_free(errmsg);
        return false;
    }
    return true;
...
bool DataManager::init_database() {
    ...
    if (!execute_sql(create_sessions)) return false;
    ...
    if (!execute_sql(create_messages)) return false;

    std::string create_index = R"(
        CREATE INDEX IF NOT EXISTS idx_messages_session_id ON messages(session_id)
    )";
    if (!execute_sql(create_index)) return false;

    LOG_INFO_STREAM() << "数据库表初始化成功";
    return true;
}
...
bool DataManager::insert_session(const Session& session) {
    ...
    const char* sql = "INSERT INTO sessions (id, model_name, created_at, updated_at) VALUES (?, ?, ?, ?)";
    sqlite3_stmt* stmt;
    int rc = sqlite3_prepare_v2(db_, sql, -1, &stmt, nullptr);
    if (rc != SQLITE_OK) {
        LOG_ERROR_STREAM() << "准备SQL失败: " << sqlite3_errmsg(db_);
        return false;
    }
    ...
    if (rc != SQLITE_DONE) {
        LOG_ERROR_STREAM() << "插入会话失败: " << sqlite3_errmsg(db_);
        return false;
    }
    LOG_DEBUG_STREAM() << "插入会话: " << session.id_;
    return true;
}
...
bool DataManager::insert_message(const std::string& session_id, const Message& msg) {
    ...
    if (rc != SQLITE_DONE) {
        LOG_ERROR_STREAM() << "插入消息失败: " << sqlite3_errmsg(db_);
        return false;
    }
    LOG_DEBUG_STREAM() << "插入消息: " << msg.id_ << " -> " << session_id;
    return true;
}
...
std::vector<Message> DataManager::get_messages(const std::string& session_id) {
    ...
    sqlite3_finalize(stmt);
    LOG_DEBUG_STREAM() << "获取消息 " << result.size() << " 条 (会话: " << session_id << ")";
    return result;
}
...
std::vector<std::shared_ptr<Session>> DataManager::get_all_sessions() {
    ...
    const char* sql = "SELECT id, model_name, created_at, updated_at FROM sessions ORDER BY updated_at DESC";
    sqlite3_stmt* stmt;
    int rc = sqlite3_prepare_v2(db_, sql, -1, &stmt, nullptr);
    if (rc != SQLITE_OK) {
        LOG_ERROR_STREAM() << "准备SQL失败: " << sqlite3_errmsg(db_);
    ...
    sqlite3_finalize(stmt);
    LOG_DEBUG_STREAM() << "加载了 " << result.size() << " 个会话";
    return result;
}
...
std::shared_ptr<Session> DataManager::get_session(const std::string& session_id) {
    ...
    const char* sql = "SELECT id, model_name, created_at, updated_at FROM sessions WHERE id = ?";
    sqlite3_stmt* stmt;
    int rc = sqlite3_prepare_v2(db_, sql, -1, &stmt, nullptr);
    if (rc != SQLITE_OK) {
        LOG_ERROR_STREAM() << "准备SQL失败: " << sqlite3_errmsg(db_);
    ...
    LOG_DEBUG_STREAM() << "获取会话: " << session_id;
    return session;
}
...
bool DataManager::update_session_timestamp(const std::string& session_id, const std::time_t &time) {
    ...
    if (rc != SQLITE_DONE) {
        LOG_ERROR_STREAM() << "更新会话时间戳失败: " << sqlite3_errmsg(db_);
        return false;
    }
    LOG_DEBUG_STREAM() << "更新会话时间戳: " << session_id;
    return true;
}
...
bool DataManager::delete_session(const std::string& session_id) {
    ...
    if (rc != SQLITE_DONE) {
        LOG_ERROR_STREAM() << "删除会话失败: " << sqlite3_errmsg(db_);
        return false;
    }
    LOG_DEBUG_STREAM() << "删除会话: " << session_id;
    return true;
}
