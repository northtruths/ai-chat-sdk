...
{
    GeminiProvider::GeminiProvider(std::shared_ptr<BaseConfig> cf)
    {
        set_model(cf);
    }

    // 设置模型配置
    bool GeminiProvider::set_model(std::shared_ptr<BaseConfig> cf)
    {
        config_ = cf;
        is_available_ = true;
        LOG_INFO("(Gemini) 模型配置设置成功");
        return true;
    }
    ...
    // 获取模型描述
    std::string GeminiProvider::get_model_desc() const
    {
        return config_->get_model_desc();
    }

    // 获取模型配置信息
    Json::Value GeminiProvider::get_params(const std::string &key) const
    {
        return config_->get(key);
    }

    // 全量式发送信息
    std::string GeminiProvider::send_message(const std::vector<Message> &messages)
    {
        // 检测模型是否有效
        if (is_available_ == false)
        {
            LOG_ERROR("(Gemini) 发送失败！模型无效！");
            return std::string();
        }
        ...
        LOG_DEBUG_STREAM() << "(gemini) 发送信息json串为：" << send_json_str;
        ...
        if (!res || res->status != 200)
        {
            if (!res)
            {
                ...
                {
                    // 没有配置好url
                    LOG_ERROR("(Gemini) 信息发送失败！原因如下：URL未配置成功，请配置 endpoint 和 path");
                }
                else
                {
                    // 网络层错误（连接失败、超时等）
        ...
        {
            // 返回信息解析失败
            LOG_ERROR_STREAM() << "(Gemini) 返回信息解析失败：" << prase_error;
            return std::string();
        }
        // 返回消息解析成功，解析大模型返回内容
        {
            ...
            // 模型返回信息错误
            LOG_ERROR("(Gemini) 返回信息错误！");
            return std::string();
        }
    }

    // 流式发送信息
    std::string GeminiProvider::send_message_stream(const std::vector<Message> &messages, one_chunk callback)
    {
        // 检测模型是否有效
        if (is_available_ == false)
        {
                ...
                if (event.substr(0, 6) == "data: ")
                {
                    ...
                    if (chunk_json.isMember("choices") && chunk_json["choices"].isArray() && !chunk_json["choices"].empty())
                    {
                        ...
                        if (choice.isMember("delta") && choice["delta"].isMember("content"))
                        {
                            ...
                            LOG_INFO_STREAM() << "(Gemini) 信息段解析成功: " << resp_chunk;
                        ...
                        {
                            LOG_DEBUG("(Gemini) json格式不符合预期, 没有具体消息");
                        }
                    }
                    else
                    {
