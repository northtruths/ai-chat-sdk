...
{
    ChatGPTProvider::ChatGPTProvider(std::shared_ptr<BaseConfig> cf)
    {
        set_model(cf);
    }

    // 设置模型配置
    bool ChatGPTProvider::set_model(std::shared_ptr<BaseConfig> cf)
    {
        config_ = cf;
        is_available_ = true;
        LOG_INFO("(ChatGPT) 模型配置设置成功");
        return true;
    }

    // 检测模型是否有效
    bool ChatGPTProvider::is_available() const
    {
        return is_available_;
    }

    // 获取模型名称
    std::string ChatGPTProvider::get_model() const
    {
        return config_->get_model();
    }
    // 获取模型描述
    std::string ChatGPTProvider::get_model_desc() const
    {
        return config_->get_model_desc();
    }

    // 获取模型配置信息
    Json::Value ChatGPTProvider::get_params(const std::string &key) const
    {
        return config_->get(key);
    }

    // 全量式发送信息
    std::string ChatGPTProvider::send_message(const std::vector<Message> &messages)
    {
        // 检测模型是否有效
        if (is_available_ == false)
        {
            LOG_ERROR("(ChatGPT) 发送失败！模型无效！");
            return std::string();
        }
        ...
        LOG_DEBUG_STREAM() << "(ChatGPT) 发送信息json串为：" << send_json_str;
        ...
        if (!res || res->status != 200)
        {
            if (!res)
            {
                ...
                {
                    // 没有配置好url
                    LOG_ERROR("(ChatGPT) 信息发送失败！原因如下：URL未配置成功，请配置 endpoint 和 path");
                }
                else
                {
                    // 网络层错误（连接失败、超时等）
                    LOG_ERROR("(ChatGPT) 信息发送失败！原因如下：网络连接失败，请检查网络或代理设置");
                }
                return std::string();
            }
            else
            {
                LOG_DEBUG_STREAM() << "ChatGPT 响应体: " << res->body;
        ...
        {
            // 返回信息解析失败
            LOG_ERROR_STREAM() << "(ChatGPT) 返回信息解析失败：" << prase_error;
            return std::string();
        }
        // 返回消息解析成功，解析大模型返回内容
        {
            ...
            // 模型返回信息错误
            LOG_ERROR("(ChatGPT) 返回信息错误！");
            return std::string();
        }
    }

    // 流式发送信息
    std::string ChatGPTProvider::send_message_stream(const std::vector<Message> &messages, one_chunk callback)
    {
        // 检测模型是否有效
        if (is_available_ == false)
        {
                                       ...
                                       uint64_t offset, uint64_t total)
        {
            if (is_error)
            {
                ...
                LOG_DEBUG_STREAM() << "ChatGPT 响应体: " << error_body;
                return true;
            }
            // 内容进入缓冲区
                ...
                if (event.substr(0, 6) == "data: ")
                {
                    ...
                    if (chunk_json.isMember("choices") && chunk_json["choices"].isArray() && !chunk_json["choices"].empty())
                    {
                        ...
                        if (choice.isMember("delta") && choice["delta"].isMember("content"))
                        {
                            ...
                            LOG_INFO_STREAM() << "(ChatGPT) 信息段解析成功: " << resp_chunk;
                        ...
                        {
                            LOG_DEBUG("(ChatGPT) json格式不符合预期, 没有具体消息");
                        }
                    }
                    else
                    {
