...
void callback(std::string chunk, bool is_end)
{
    if (is_end)
    {
        return;
...
int main()
{
    ...
    std::string gpt_key = env_config["chatgpt_api_key"].asString();
    ...
    //  3. 准备模型配置
    // ============================================================
    auto configs = chat_sdk.get_default_configs();

    // 3.1  模型配置
    configs[0]->set_api_key(deepseek_key);
    configs[1]->set_api_key(gpt_key);
    configs[2]->set_api_key(gemini_key);
    ...
    //  5. 创建会话
    // ============================================================
    std::string session_id = chat_sdk.create_session("deepseek");
    if (session_id.empty())
    {
        LOG_ERROR("创建会话失败");
        return -1;
    ...
    {
        std::cout << model._provider << " - " << model._modelName << std::endl;
        std::cout << model._modelDesc << std::endl;
    }
